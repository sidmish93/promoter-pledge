...
_DROP = {"limited", "ltd", "pvt", "private", "the", "co", "company"}
...
def match_score(a: str, b: str) -> float:
    """How well two promoter strings refer to the same person. 0–100."""
    parts_a = re.split(r"\s*/\s*|\s+&\s+|\s+and\s+", str(a or ""), flags=re.I)
    if len(parts_a) > 1:
        return max(match_score(p, b) for p in parts_a if p.strip())
...
class NseLive:
    ...
    def _row_from_nse_payload(self, row: dict) -> dict | None:
        ...
        rec["name"] = name
        rec["nse_symbol"] = str(row.get("symbol") or "").strip().upper()
    ...
    def pledged_companies(self) -> list[dict]:
        ...
        for rec in self._pledged_rows():
            ...
            row = {
                "name": rec["name"],
                "nse_symbol": rec.get("nse_symbol") or "",
    ...
    def events(self, lookback_days: int = EVENT_LOOKBACK_DAYS) -> list[dict]:
        ...
        out = []
        for row in rows or []:
            event_date = parse_date(row.get("eventDetailsToDate"))
            broadcast = parse_date(row.get("broadcastdate"))
            out.append(
                {
                    "nse_symbol": str(row.get("symbol") or "").strip().upper(),
                    "company": str(row.get("companyName") or "").strip(),
                    "promoter": str(row.get("promoterName") or "").strip(),
                    "event_type": str(row.get("eventDetailsType") or "").strip(),
                    "encumbrance_type": str(row.get("eventDetailsTypeEncumb") or "").strip(),
                    "lender": str(row.get("eventDetailsEntity") or "").strip(),
                    "event_shares": to_num(row.get("eventDetailsHolding")),
                    "event_pct_equity": to_num(row.get("eventDetailsPerc")),
                    ...
                    "post_event_encumbered_shares": to_num(row.get("postEventHolding")),
                    "post_event_encumbered_pct": to_num(row.get("postEventHoldingPerc")),
                    "source": "NSE",
...
class BseLive:
    ...
    def pledged_companies(self) -> list[dict]:
        """BSE last-quarter promoter pledges only.

        Saved list first so Start does not hang on BSE's slow pledge page.
        """
    ...
    def shp_walk(self, scripcode: str, latest: dict, promoters: list[dict]) -> None:
        ...
        names = [p["name"] for p in promoters]
        ids = _walk_qids(latest_qid)
        tables: dict[int, list[dict]] = {latest_qid: [dict(p) for p in promoters]}
        ...
        for qid in missed:
            for alt in (qid - 1, qid + 1):
                if 0 < alt < latest_qid and alt not in seen:
                    ...
                    seen.add(alt)
        extra = _gap_qids(ids, names, tables) + neighbors
        ...
        for p in promoters:
            ...
            for qid in ordered:
                ...
                qname = quarter_label(month, year)
                row = _match_holder(p["name"], tables.get(qid) or [])
...
def _pledged_key(name: str, rows: list[dict]):
    row = _match_holder(name, rows)
    if not rows:
        return ("empty", None)
    if not row:
        return ("absent", None)
...
def _gap_qids(ids: list[int], names: list[str], tables: dict[int, list[dict]]) -> list[int]:
    ...
    for newer, older in zip(ordered, ordered[1:]):
        ...
        if newer not in wanted and older not in wanted:
            continue
        changed = any(
            _pledged_key(name, tables.get(newer) or []) != _pledged_key(name, tables.get(older) or [])
            for name in names
        )
        if changed:
...
def _match_holder(name: str, rows: list[dict]) -> dict | None:
    best, score = None, 0.0
    for row in rows:
        s = match_score(name, row.get("name") or "")
        if s > score:
            best, score = row, s
    return best if score >= 65 else None
...
def _measure(sessions: list[dict]) -> dict:
    ...
    for label, days in METRIC_WINDOWS:
        ...
        disclosed = [s for s in inside if s["delivery_qty"] is not None]
...
def nse_archives_history(symbol: str, calendar_days: int = 45) -> list[dict]:
    ...
    def one(day: date) -> dict | None:
        ...
        for rec in csv.DictReader(StringIO(r.text)):
            ...
            if series not in NSE_SERIES:
                continue
            close = to_num(_csv_col(rec, "close_price"))
            qty = to_num(_csv_col(rec, "ttl_trd_qnty"))
            if not close or close <= 0 or not qty or qty <= 0:
                continue
            lacs = to_num(_csv_col(rec, "turnover_lacs"))
            delivered = _csv_col(rec, "deliv_qty")
            when = parse_date(_csv_col(rec, "date1")) or day
            return {
                "day": when,
                "close": close,
                "quantity": qty,
    ...
    with ThreadPoolExecutor(max_workers=10) as pool:
        for fut in [pool.submit(one, day) for day in days]:
            try:
                row = fut.result()
            except Exception:
                continue
...
def _merge_market_sessions(yahoo: list[dict], nse_rows: list[dict]) -> list[dict]:
    ...
    if not nse_rows:
        return yahoo
    extra = {s["day"]: s for s in nse_rows}
    merged = []
    for s in yahoo:
        rec = dict(s)
        hit = extra.get(s["day"])
        if hit and hit.get("delivery_qty") is not None:
            rec["delivery_qty"] = hit["delivery_qty"]
            rec["quantity"] = hit["quantity"]
            rec["value"] = hit["value"]
        merged.append(rec)
    return merged
...
def load_company(
    ...
    symbol = (symbol or snap.get("nse_symbol") or "").strip().upper()
    scripcode = str(scripcode or "").strip()
